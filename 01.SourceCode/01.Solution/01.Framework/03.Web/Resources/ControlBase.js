...
var $HGRootNSName = 'Framework.Web.Controls';
...
$HGRootNS.BehaviorBase.prototype = {
	...
	_invoke: function(name, args, cb, errCb, bSync) {
		/// <summary>
		///继承子对象调用此方法，对服务端控件方法进行调用，通过回调函数cb对调用结果进行处理
		...
		try {
			...
			var payload = Sys.Serialization.JavaScriptSerializer.serialize({ name: name, args: ar, state: clientState });
			WebForm_DoCallback(this._callbackTarget, payload, this._oncomplete$delegate, [cb, errCb], this._onerror$delegate, false, bSync, this._invokeWithoutViewState);
		}
		catch (e) {
			this._isInvoking = false;
			throw (e);
