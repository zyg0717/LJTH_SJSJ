...
$HGRootNS.DeluxeGrid = function(element) {
	...
	this._showCheckBoxs = false; //是否启用checkbox选择框
...
$HGRootNS.DeluxeGrid.prototype =
{
	initialize: function() {
		...
		if (this._showCheckBoxs && this._isChecked) {
	...
	_initializeControls: function() {
		...
		if (this._showPager && this._showExportControl) {
			this._exportExcelTopButton = $get(this._excelTopButtonClientID);
			this._exportExcelButton = $get(this._excelButtonClientID);
			this._exportWordTopButton = $get(this._wordTopButtonClientID);
			this._exportWordButton = $get(this._wordButtonClientID);

			this._btnWordServer = $get(this._btnWordClientID);
			this._btnExcelServer = $get(this._btnExcelClientID);
	...
	set_showCheckBoxs: function(value) {
		this._showCheckBoxs = value;
	},

	get_showCheckBoxs: function() {
		return this._showCheckBoxs;
	},
	...
	*/
			...
			try {
				...
				if (cmd == "word") {
					if (this._btnWordServer)
						this._btnWordServer.click();
				}
				else {
					if (this._btnExcelServer)
						this._btnExcelServer.click();
				}
	...
	_initPostFrame: function() {
		...
		if (!fc) {
			...
			$HGDomElement.get_currentDocument().appendChild(fc);
		}
