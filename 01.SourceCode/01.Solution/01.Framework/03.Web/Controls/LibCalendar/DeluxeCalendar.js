...
$HGRootNS.DeluxeCalendar = function(element) {
    ...
    this._imageButtonPath = "<%=WebResource("Framework.Web.Controls.LibCalendar.Images.datePicker.gif")%>";
...
//           return;
			this._imageButton = $HGDomElement.createElementFromTemplate
  					(
		  				{
		  					nodeName : "img",
		  					properties : {src : this._imageButtonPath, id: elt.id + "_image", type : "image", align : 'absmiddle' },
	//		  				style : { align : "absmiddle" },
		  					//events : this._picker$delegates, //
		  					cssClasses : ["ajax_calendarimagebutton"]
		  				},
		  				elt.parentElement
  					);
//  			elt.insertAdjacentElement("afterEnd", this._imageButton);
        }
...
//        $HGDomElement.setDynamicLocation(elt, this._imageButton, this._setImageButtonLocation);
    ...
    //check date
     ,_captureValidatorsControl : function(str)
     {
		...
		return ar[0] != 0000 && d.getMonth() == ar[1] && d.getDate() == ar[2];  
     }
...
//                $HGDomElement.createElementFromTemplate({ nodeName : "span", properties : { innerText : "2b "}}, this._header);   
   ...
   //?????             
    ...
    _ensureCalendar : function() 
    {
        if (!this._container) {
            ...
            var elt = this.get_element();
            //popoup size 200,200
            this._calendarPopup = $create($HGRootNS.PopupControl, { positionElement : elt, positioningMode : $HGRootNS.PositioningMode.BottomLeft}, {}, {}, null);         
   ...
   ,_onKeyPress : function(args)
   {
    ...
    if(!this._isInsideImg)  //true不校验
    {
      ...
      {
        var IsValid = this._captureValidatorsControl(ValueText);
