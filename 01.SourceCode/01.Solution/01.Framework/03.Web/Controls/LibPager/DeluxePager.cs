...
namespace Framework.Web.Controls
{
    ...
    ///  DeluxePager控件 继承子ScriptControlBase
    /// </remarks>
    [DefaultProperty("DeluxePager"),    ToolboxData("<{0}:DeluxePager runat=server Width=\"500\"></{0}:DeluxePager>")]
    [Designer(typeof(DeluxePagerDesigner))]
    [ParseChildren(true),
    PersistChildren(false)]
    public class DeluxePager : WebControl, IPostBackEventHandler, IPostBackContainer, INamingContainer //ScriptControlBase
    {
        /// <summary>
        /// 构造函数
        /// </summary>
        ...
        */
        ...
        protected override void OnInit(EventArgs e)
        {
            if (!this.DesignMode)
            {
                ...
                if (this.control != null)
                {
                    ...
                    {
                        //DataGrid与其它具有翻页功能控件的机制不同继承的基类不同这里排除DataGrid类型
        ...
        void SetPageIndex(int pageIndex)
        {
            ...
            {
                PageIndex = pageIndex;
                this.DeluxePager_CommonPageIndexChanged();
            }
        }
        ...
        private void DrawingPageNumber(TableCell tc)
        {
            ...
            switch ((PagerCodeShowMode)PageCodeShowMode)
            {
                case PagerCodeShowMode.All:
                    ...
                    {
                        strB.Append("&nbsp;&nbsp;");
                        strB.Append(Translator.Translate(Define.DefaultCategory, "总记录数{0}", pageInfo.RecordCount));
                        strB.Append("&nbsp;&nbsp;");
                        strB.Append(Translator.Translate(Define.DefaultCategory, "{0}/页", pageInfo.PageSize));
                        strB.Append("&nbsp;&nbsp;");
                    }

                    strB.Append(Translator.Translate(Define.DefaultCategory, "第{0}页", pageInfo.CurrentPage));
                    strB.Append("&nbsp;");
                    strB.Append(Translator.Translate(Define.DefaultCategory, "共{0}页", pageInfo.PageCount));
        ...
        private void DrawingPageNumber(TableCell tc)
        {
            ...
            {
                case PagerCodeShowMode.All:
                    tc.Controls.Add(new LiteralControl(string.Format("&nbsp;&nbsp;{0}", 
                        Translator.Translate(Define.DefaultCategory, "总记录数"))));

                    if (RecordCount.HasValue)
                    {
                        tc.Controls.Add(this.recordCount);
                        tc.Controls.Add(new LiteralControl("&nbsp;&nbsp;"));
                        tc.Controls.Add(this.pageSize);
                        tc.Controls.Add(new LiteralControl(Translator.Translate(Define.DefaultCategory, "/页&nbsp;&nbsp;第")));
                    }
                    tc.Controls.Add(this.currentPage);
                    tc.Controls.Add(new LiteralControl(string.Format("{0}", 
                        Translator.Translate(Define.DefaultCategory, "页/共"))));

                    tc.Controls.Add(this.pageCount);
                    tc.Controls.Add(new LiteralControl(string.Format("{0}&nbsp;&nbsp;&nbsp;&nbsp;", 
                        Translator.Translate(Define.DefaultCategory, "页"))));
                    break;
                case PagerCodeShowMode.CurrentRecordCount:
                    if (RecordCount.HasValue)
                    {
                    ...
                    tc.Controls.Add(this.currentPage);
                    tc.Controls.Add(new LiteralControl("页/共"));
                    tc.Controls.Add(this.pageCount);
                    tc.Controls.Add(new LiteralControl("页&nbsp;&nbsp;&nbsp;&nbsp;"));
                    break;
                case PagerCodeShowMode.RecordCount:
                    tc.Controls.Add(new LiteralControl("&nbsp;&nbsp;总记录数"));
                    if (RecordCount.HasValue)
                    {
                        tc.Controls.Add(this.recordCount);
                        tc.Controls.Add(new LiteralControl("&nbsp;&nbsp;"));
        ...
        ///  绑定控件的方法
        /// </remarks>
        void BoundControl_DataBound(object sender, EventArgs e)
        {
        ...
        private void DeluxePager_CommonPageIndexChanged()
        {
        ...
        private void HandleEvent(CommandEventArgs args)
        {
            if (args != null)
            {
                ...
                {
                    int pageIndex = 0;
                    bool b = int.TryParse((string)args.CommandArgument, out pageIndex);
                    if (b)
                ...
                {
                    int pageIndex = 0;
                    bool b = int.TryParse(Page.Request.Form[this.UniqueID + "$txtPageCode"], out pageIndex);
                    if (b)
                        this.PageIndex = pageIndex - 1;
                }
