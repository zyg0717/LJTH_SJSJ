...
{
    [Serializable]
    [ORViewMapping(@" (select T.ID as TID,k.ID,T.TemplateConfigInstanceName,k.DataCollectUserID,k.CreatorLoginName,k.CreatorName,k.CreatorTime,k.Status  
                    from dbo.TemplateConfigInstance t
                    join dbo.TemplateTask k on t.ID=k.TemplateConfigInstanceID 
                    where t.IsDeleted=0 and k.IsDeleted=0 and k.ProcessStatus=0) "
            , "TaskUser")]
    public class VTaskUser : IBaseComposedModel
    {
        [ORFieldMapping("ID")]
